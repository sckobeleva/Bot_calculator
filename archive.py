# первая версия калькулятора (простой калькулятор, 4 возможных действия, натуральные числа)
...
def second_argument_and_operation(message):    # получаем второй аргумент
    ...
    while b is not True:
        ...
        second = message.text
        #bot.send_message(message.from_user.id, 'Напиши операцию')
        # bot.send_message(message.from_user.id, first) # проверка для отладки, что сохраняется правильное последнее значение
        keyboard = types.InlineKeyboardMarkup()  # наша клавиатура
